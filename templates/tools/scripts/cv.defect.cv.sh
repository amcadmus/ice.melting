...
# optional arguments:
	...
	-q|--loc-cv dir  (steinhardt)		tell the dir of the Q6 and CV. 
       -t1|--thres1 val  (0.358)		tell the threshold 1 of the Q6. 
       -t1|--thres2 val  (0.267)		tell the threshold 2 of the Q6. 
       -tc|--thresc val  (0.5)			tell the threshold of the CV
...
echo conf: $file_conf
echo traj: $file_traj
echo dir loc-cv: $file_q
echo thres q6 1: $thres1
echo thres q6 2: $thres2
echo thres cv: $thresc
...
prog2=awkprog2.$$
prog3=awkprog3.$$
echo "{if (\$2 < $thres1 && \$2 > $thres2 && \$3 <  $thresc) {print \$1}}" > $prog1
echo "{if (\$2 < $thres1 && \$2 > $thres2 && \$3 >= $thresc) {print \$1}}" > $prog2
echo "{if (\$2 < $thres2) {print \$1}}" > $prog3
for ii in $targets; 
do
...
#	pjj=`printf %f $jj | cut -d '.' -f 1`
    ...
    count_frame3=0
    list3=""
    for jj in `grep -v \# $file_q/$ii | awk -f $prog3`;
    do
	pjj=$jj
